# -*- coding: utf-8 -*-
"""
Created on Sat Apr 27 08:35:19 2019
...
df =pd.read_csv('gasUsagecylinder.csv')
...
def test_staionary(timeseries):
    ...
    print('Results of Dicky Fullers Test')
    dftest = adfuller(timeseries['Cylinders'], autolag = 'AIC')
...
data = (results_AR.fittedvalues-df_logshifting['Cylinders'])**2
...
data = (results_MA.fittedvalues-df_logshifting['Cylinders'])**2
plt.title(data.sum(axis =0, skipna=True))
...
data = (results_ARIMA.fittedvalues-df_logshifting['Cylinders'])**2
plt.title(data.sum(axis =0, skipna=True))
...
predictions_ARIMA_log = pd.Series(indexedDataset_logscale['Cylinders'].ix[0],index=indexedDataset_logscale.index)
...
def predict_gasUsage(date):
    ...
    noOfCylinders = np.asarray(df_predcited[0][prediction_index - 1]*1000, dtype=np.int32)
    print("You predicted Gas Usage for ", date.strftime("%B"), " is ",noOfCylinders , "Cylinders")
    #print(noOfCylinders)
    return noOfCylinders

#test
print(predict_gasUsage(datetime.now()))
